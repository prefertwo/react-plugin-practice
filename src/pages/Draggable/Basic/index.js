...
export default class DraggableComponent extends React.Component {
  ...
  render() {
    ...
    return (
      // PageHeaderWrapper 包含面包屑组件，否组不会有面包屑导航
      <PageHeaderWrapper>
        <Row>
          <Col span={16} offset={4}>
            <Alert
              ...
              message={source.map(item => (
                <a key={item.id} target="_blank" href={item.href} style={{ marginRight: 40 }}>
                  {' '}
                  {item.title}{' '}
                </a>
              ))}
            ...
            >
              ...
              <Draggable {...options} handle=".handle" bounds="parent">
                <div className="handle" style={style}>
                  我可以在父元素内 被任意拖拽 <br /> 父元素需设置相对定位属性{' '}
                </div>
              </Draggable>

              <Draggable {...options} handle=".handle" axis="x">
                <div className="handle" style={style}>
                  我只能在X轴拖拽
                </div>
              </Draggable>

              <Draggable {...options} handle=".handle" axis="y">
                <div className="handle" style={style}>
                  我只能在Y轴拖拽
                </div>
              </Draggable>
              ...
              <Draggable onDrag={this.showPosition}>
                <div className="handle" style={style}>
              ...
              <Draggable {...options} grid={[20, 30]}>
                <div style={style}>移动步长为20，30</div>
              </Draggable>

              <Draggable {...options} bounds={{ top: -100, right: 100, bottom: 100, left: -100 }}>
                <div style={style}>只能在方圆200内移动</div>
              </Draggable>
            </div>
